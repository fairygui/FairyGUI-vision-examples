...
void BasicsScene::continueInit()
{
    ...
    _backBtn->addClickListener(CALLBACK_1(BasicsScene::onClickBack, this));
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        if (obj->getGroup() != nullptr && obj->getGroup()->name.compare("btns") == 0)
            obj->addClickListener(CALLBACK_1(BasicsScene::runDemo, this));
    }
}
...
void BasicsScene::playText()
{
    ...
    obj->getChild("n12")->addListener(UIEventType::ClickLink, [this](EventContext* context)
    {
...
void BasicsScene::playPopup()
{
    if (_pm == nullptr)
    {
        ...
        _pm->retain();
        _pm->addItem("Item 1", CALLBACK_1(BasicsScene::onClickMenu, this));
        _pm->addItem("Item 2", CALLBACK_1(BasicsScene::onClickMenu, this));
        _pm->addItem("Item 3", CALLBACK_1(BasicsScene::onClickMenu, this));
        _pm->addItem("Item 4", CALLBACK_1(BasicsScene::onClickMenu, this));
    }
    ...
    obj->addListener(UIEventType::RightClick, [this](EventContext* context)
    {
        _pm->show();
    });
}
...
void BasicsScene::playDepth()
{
    ...
    obj->getChild("btn0")->addClickListener([obj](EventContext*)
    {
        ...
        graph->drawRect(150, 150, 1, V_RGBA_BLACK, V_RGBA_RED);
    ...
    obj->getChild("btn1")->addClickListener([obj](EventContext*)
    {
        ...
        startPos.y += 10;
        graph->setPosition(startPos.x, startPos.y);
        graph->drawRect(150, 150, 1, V_RGBA_BLACK, V_RGBA_GREEN);
...
void BasicsScene::playDragDrop()
{
    ...
    b->setDraggable(true);
    b->addListener(UIEventType::DragStart, [this,b](EventContext* context)
    {
        //Cancel the original dragging, and start a new one with a agent.
        context->preventDefault();

        _groot->getDragDropManager()->startDrag(b->getIcon(), Value(b->getIcon()), context->getInput()->getTouchId());
    });

    GButton* c = obj->getChild("c")->as<GButton>();
    c->setIcon("");
    c->addListener(UIEventType::Drop, [c](EventContext* context)
    {
        c->setIcon(context->getDataValue().asString());
    });

    GObject* bounds = obj->getChild("n7");
    VRectanglef rect = bounds->transformRect(VRectanglef(hkvVec2::ZeroVector(), bounds->getSize()), _groot);
    ...
    rect.m_vMin.x -= obj->getParent()->getX();
    rect.m_vMax.x -= obj->getParent()->getX();
    //----
