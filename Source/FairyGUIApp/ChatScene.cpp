...
void ChatScene::continueInit()
{
    ...
    _list->setVirtual();
    _list->itemProvider = CALLBACK_1(ChatScene::getListItemResource, this);
    _list->itemRenderer = CALLBACK_2(ChatScene::renderListItem, this);

    _input = _view->getChild("input")->as<GTextInput>();
    _input->addListener(UIEventType::Submit, CALLBACK_1(ChatScene::onSubmit, this));

    _view->getChild("btnSend")->addClickListener(CALLBACK_1(ChatScene::onClickSendBtn, this));
    _view->getChild("btnEmoji")->addClickListener(CALLBACK_1(ChatScene::onClickEmojiBtn, this));

    _emojiSelectUI = UIPackage::createObject("Emoji", "EmojiSelectUI")->as<GComponent>();
    _emojiSelectUI->retain();
    _emojiSelectUI->getChild("list")->addListener(UIEventType::ClickItem, CALLBACK_1(ChatScene::onClickEmoji, this));
}
...
void ChatScene::renderListItem(int index, GObject * obj)
{
    ...
    tf->setText("");
    tf->setWidth(tf->initSize.x);
    tf->setText(EmojiParser::defaultParser.parse(msg.msg.c_str()));
